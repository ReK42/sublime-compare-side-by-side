# -*- coding: utf-8 -*-
"""A simple side-by-side comparison tool to Sublime Text 2 and 3."""
import os
import re
import difflib
import json
...
def plugin_loaded():
    """Will be called when the plugin is ready to use.

    Creating these files before they're needed avoids some weird issues
    where sublime insists the file doesn't exist, when it definitely does
    """
    ...
    with open(
        os.path.join(folder, "SBSCompareTheme.hidden-tmTheme"),
        "w",
        encoding="utf-8",
    ) as f:
        f.write("")
    ...
    with open(
        os.path.join(folder, "SBSCompareScheme.hidden-color-scheme"),
        "w",
        encoding="utf-8",
    ) as f:
        f.write("")

    """Delete old non-hidden files. Prevents them from showing up in
    Preferences -> Color Scheme... selection dialog, see PR #53. Also, if those files
    were malformed (0 bytes, as a result of the bug fixed with PR #55) opening that
    dialog threw this error message:
...
class SbsCompareCommand(sublime_plugin.TextCommand):
    def generate_colour_scheme(self, view, generate=True):
        ...
        colour_removed = sbs_settings().get(
            "remove_colour",
            "invalid.illegal",
        )
        colour_added = sbs_settings().get(
            "add_colour",
            "string",
        )
        colour_modified_deletion = sbs_settings().get(
            "modified_colour_deletion",
            "support.class",
        )
        colour_modified_addition = sbs_settings().get(
            "modified_colour_addition",
            "support.class",
        )
        colour_unmodified_deletion = sbs_settings().get(
            "unmodified_colour_deletion",
            "invalid.illegal",
        )
        colour_unmodified_addition = sbs_settings().get(
            "unmodified_colour_addition",
            "string",
        )
        colour_text = sbs_settings().get(
            "text_colour",
            "",
        )
        ...
        for col in [
            colour_removed,
            colour_added,
            colour_modified_deletion,
            colour_modified_addition,
            colour_unmodified_deletion,
        ...
        if generate:
            ...
            try:
                ...
                """Sometimes load_resource can fail (seemingly on OSX when upgrading
                from ST2->ST3). Manually re-selecting the colour scheme once should fix
                this for good (see issue #31).
                """
                ...
                scheme = (
                    ...
                    '<plist version="1.0">',
                    "<dict><key>settings</key><array></array></dict>",
                    "</plist>",
                )
            ...
            if scheme_json:
                ...
                for name in colourStrings:
                    ...
                    chex = colourHexes[name]
                    xml += xml_tmpl.format(
                        "Comparison " + name, string, chex, colour_text
                    )
                # Combiiiiiiiiiiiiine
    ...
    def get_current_color_scheme(self, view):
        ...
        """The default 'color_scheme' setting only has the file name and it's inside
        the 'Color Scheme - Default' package.
        """
        if not current_scheme.startswith(packages_path + "/"):
            rel_current_scheme_file = os.path.join(
                packages_path, "Color Scheme - Default", current_scheme
            )
...
class SbsSelectTextCommand(sublime_plugin.TextCommand):
    def run(self, edit, index=""):
        ...
        combined_regions = []
        """Hacky but necessary to combine regions both start==end AND end==start there's
